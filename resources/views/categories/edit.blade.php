...
<div class="bg-white rounded-lg shadow p-6 max-w-2xl">
    <form action="{{ route('categories.update', $category) }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="number" step="0.01" name="percentage" value="{{ old('percentage', $category->percentage) }}" class="shadow border rounded w-full py-2 px-3 text-gray-700" required min="0.01" max="{{ $availablePercentage + $category->percentage }}">
            <p class="text-gray-600 text-sm mt-1">Available: {{ number_format($availablePercentage, 2) }}% (excluding current)</p>
        </div>
